...
class FeedbackInterface:
    def __init__(
        ...
        **kwargs,
        ...
        self.generated_image_size = None # size of each image (generated by SD)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.feedback_type == FeedbackTypes.SCORE_ONE:
            score_options = np.arange(self.feedback_args["score_range"][0], self.feedback_args["score_range"][1])
            self.reference_images = {score : None for score in score_options}
    ...
    def query_batch(self, prompts, image_batch, query_indices, **kwargs):
        ...
        for query, prompt in zip(query_indices, prompts):
            ...
            self._save_query_image(
                images=images,
                prompt=prompt,
                img_save_path="real_human_query_image.png",
            )

            # if feedback type is score_one, save reference image (most recent image given each score)
            if self.feedback_type == FeedbackTypes.SCORE_ONE:
                self._save_query_image(
                    images=list(self.reference_images.values()),
                    prompt=prompt,
                    img_save_path="real_human_reference_image.png",
                )
            ...
            # if feedback type is score_one, update reference image
            if self.feedback_type == FeedbackTypes.SCORE_ONE:
